...
<?php 
	...
	function lay_de_xuat_du_an(){
		...
		$conn = $ketnoi->ketnoi();
		$query = "SELECT dx.IDDT, dt.MADETAI,`TENDETAI` FROM dexuatdetai dx, detai dt WHERE dx.IDDT = dt.IDDT AND (dt.TRANGTHAI = N'Đang thực hiện' OR dt.TRANGTHAI = N'Đã nghiệm thu') ORDER BY dx.IDDX DESC";
 ...
 <div class="card">
 	<div class="card-header">
 		<h4>Phiếu xác nhận triển khai ứng dụng</h4>
		    ...
		    $detai = lay_de_xuat_du_an();
		     while ($row = mysqli_fetch_assoc($detai)){ ?>
		        <tr>
		            ...
		            <td class="giua">
		                <a href="word/phieu_xac_nhan_trien_khai_ung_dung.php?id=<?php echo $row['IDDT']; ?>" target="_blank" class="btn btn-warning btn-sm"><i class="far fa-file-word"></i> Xuất file</a>
		            </td>
		        </tr>
		    <?php $stt++; } ?>
		    </tbody>
