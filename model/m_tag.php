<?php 
	...
	function tim_tag($tu){
	    ...
	    $conn = $ketnoi->ketnoi();
	    $query = "SELECT bt.IDBAO AS ID,TENBAO AS TEN, tk.TENKHOA AS TENKHOA,'bao' AS LOAI FROM baokhoahoc bkh, baibao_tukhoa bt, tukhoa tk WHERE bt.IDKHOA = N'$tu' AND  bt.IDBAO = bkh.IDBAO AND tk.IDKHOA=bt.IDKHOA UNION  ALL SELECT bv.IDBV AS ID,TENBV AS TEN, tk2.TENKHOA AS TENKHOA,'tin' AS LOAI FROM baiviet bv, baiviet_tukhoa bt2, tukhoa tk2 WHERE bt2.IDKHOA = N'$tu' AND bt2.IDBV = bv.IDBV AND tk2.IDKHOA=bt2.IDKHOA GROUP BY ID, TEN, TENKHOA, LOAI";
