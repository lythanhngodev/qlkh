<?php 
	...
	function lay_tin($id){
		...
		$conn = $ketnoi->ketnoi();
		$query = "SELECT bv.IDBV, bv.TENBV, bv.LINKBV, bv.HINHANH from baiviet bv, chuyenmuc cm, baiviet_chuyenmuc bc WHERE bc.IDBV = bv.IDBV AND bc.IDCM = cm.IDCM AND cm.LOAICHUYENMUC=N'hoptacquocte' AND bv.HIENTHI=b'1' AND bc.IDCM='$id' ORDER BY bv.IDBV DESC;";
	...
	function so_tin($id){
		$ketnoi = new clsKetnoi();
		$conn = $ketnoi->ketnoi();
		$query = "SELECT * from baiviet bv, chuyenmuc cm, baiviet_chuyenmuc bc WHERE bc.IDBV = bv.IDBV AND bc.IDCM = cm.IDCM AND cm.LOAICHUYENMUC=N'hoptacquocte' AND bv.HIENTHI=b'1' AND bc.IDCM='$id';";
		$result = mysqli_query($conn, $query);
		mysqli_close($conn);
	...
	function lay_tin_them($id,$bd,$sotin){
		$ketnoi = new clsKetnoi();
		$conn = $ketnoi->ketnoi();
		$query = "SELECT bv.IDBV, bv.TENBV, bv.LINKBV, bv.HINHANH,NGAYDANG from baiviet bv, chuyenmuc cm, baiviet_chuyenmuc bc WHERE bc.IDBV = bv.IDBV AND bc.IDCM = cm.IDCM AND cm.LOAICHUYENMUC=N'hoptacquocte' AND bv.HIENTHI=b'1' AND bc.IDCM='$id' ORDER BY bv.IDBV DESC LIMIT $bd, $sotin";
		$result = mysqli_query($conn, $query);
		mysqli_close($conn);
		return $result;
	}
